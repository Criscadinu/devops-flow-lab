    ...
    <div className="flex-1 px-6 py-14">
      <div className="max-w-3xl mx-auto flex flex-col gap-8">
        ...
        <div className="flex flex-col gap-2">
          <h2 className="text-3xl text-white tracking-tight" style={{ ...syne.style, fontWeight: 800 }}>Your Mission - Remove the Wall</h2>
        ...
        <TaskCard number="01" title="Document who owns what" done={task1Done} locked={false}>
          <MentorNote>
            <p className="text-sm text-gray-300 leading-relaxed"><span className="text-white">Shared ownership starts with explicit ownership.</span>{" "}If nobody owns it, nobody fixes it. A CODEOWNERS file makes ownership visible in every PR.</p>
          </MentorNote>
          <div className="flex flex-col gap-2">
...
.github/workflows/ @ops-team @dev-team`}</CodeBlock>
          ...
          {!task1Done && (
            <label className="flex items-center gap-3 cursor-pointer group">
              ...
              <span className="text-sm text-gray-400 group-hover:text-gray-300 transition-colors">CODEOWNERS file created — ownership is explicit</span>
            </label>
          )}
        </TaskCard>
        ...
        <TaskCard number="02" title="Add operational metadata to the app" done={task2Done} locked={!task1Done}>
          <MentorNote>
            <p className="text-sm text-gray-300 leading-relaxed"><span className="text-white">Ops needs to understand what is running in production.</span>{" "}Build operational visibility into the app itself — version, uptime, memory. Marco should not have to ask Lisa what is deployed.</p>
          </MentorNote>
          <div className="flex flex-col gap-2">
    ...
    timestamp: new Date().toISOString(),
          ...
          {!task2Done && (
            <label className="flex items-center gap-3 cursor-pointer group">
              ...
              <span className="text-sm text-gray-400 group-hover:text-gray-300 transition-colors">Health endpoint exposes operational metadata</span>
            </label>
          )}
        </TaskCard>
        ...
        <TaskCard number="03" title="Create a runbook entry for the orders service" done={task3Done} locked={!task2Done}>
          <MentorNote>
            <p className="text-sm text-gray-300 leading-relaxed"><span className="text-white">A runbook written by the developer who built the feature is more useful than one written by the ops engineer who had to debug it at 2am.</span>{" "}Write it now, before anything goes wrong.</p>
          </MentorNote>
          <div className="flex flex-col gap-2">
...
4. Page dev team if unresolved after 30 minutes`}</CodeBlock>
          ...
          {!task3Done && (
            <label className="flex items-center gap-3 cursor-pointer group">
              ...
              <span className="text-sm text-gray-400 group-hover:text-gray-300 transition-colors">Runbook entry written by developer — operational knowledge in the repo</span>
            </label>
          )}
        </TaskCard>
        ...
        <TaskCard number="04" title="Add a memory leak test" done={task4Done} locked={!task3Done}>
          <MentorNote>
            <p className="text-sm text-gray-300 leading-relaxed"><span className="text-white">The memory leak Marco fixed repeatedly was never caught by a test.</span>{" "}Operational concerns are testable. Add a test that would have caught it before it reached Marco&apos;s 2am alert.</p>
          </MentorNote>
          <div className="flex flex-col gap-2">
  ...
  expect(memoryMB).toBeLessThan(100) // Fail if heap exceeds 100MB
          ...
          {!task4Done && (
            <label className="flex items-center gap-3 cursor-pointer group">
              ...
              <span className="text-sm text-gray-400 group-hover:text-gray-300 transition-colors">Memory leak test added — operational concern is now a test</span>
            </label>
          )}
        </TaskCard>
        ...
        <TaskCard number="05" title="Commit and push" done={task5Done} locked={!task4Done}>
          <MentorNote>
            <p className="text-sm text-gray-300 leading-relaxed"><span className="text-white">Ownership is explicit. The app speaks for itself. The runbook is in the repo. The memory leak test will never let Marco fix the same bug twice.</span></p>
          </MentorNote>
          <div className="flex flex-col gap-2">
          ...
          {!task5Done && (
            <div className="flex flex-col gap-3">
              ...
              <label className="flex items-center gap-3 cursor-pointer group">
                ...
                <span className="text-sm text-gray-400 group-hover:text-gray-300 transition-colors">Pipeline is green</span>
              </label>
            </div>
          )}
        </TaskCard>
