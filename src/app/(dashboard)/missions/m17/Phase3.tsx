    ...
    <div className="flex-1 px-6 py-14">
      <div className="max-w-3xl mx-auto flex flex-col gap-8">
        ...
        <div className="flex flex-col gap-2">
          <h2 className="text-3xl text-gray-900 tracking-tight" style={{ fontFamily: "var(--font-heading)", fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.02em" }}>Your Mission - Make Deploying Self-Service</h2>
          <p className="text-sm leading-relaxed" style={{ color: "var(--text-muted)" }}>Add a manual trigger, automate the deployment job, add tags, and write the deployment guide.</p>
        </div>
        ...
        <div className="flex flex-col gap-5 p-6 border" style={{ backgroundColor: "var(--bg-card)", borderColor: "rgba(251,146,60,0.3)", borderLeft: "3px solid rgb(251,146,60)" }}>
          ...
          <p className="text-sm leading-relaxed" style={{ color: "var(--text-muted)" }}>This mission builds on M-16. Your pipeline should already run tests automatically on every push to main.</p>
        </div>

        <TaskCard number="01" title="Add a manual deployment trigger to CI" done={task1Done} locked={false}>
          <MentorNote>
            <p className="text-sm text-gray-600 leading-relaxed"><span className="text-gray-900">workflow_dispatch adds a Run Workflow button to the Actions tab.</span>{" "}Any engineer with repo access can trigger it. This is the first step toward self-service — the pipeline does the work, the engineer presses the button.</p>
          </MentorNote>
          <div className="flex flex-col gap-2">
    ...
    inputs:
      environment:
        ...
        options:
          ...
          {!task1Done && (
            <label className="flex items-center gap-3 cursor-pointer group">
              ...
              <span className="text-sm transition-colors" style={{ color: "var(--text-muted)" }}>workflow_dispatch trigger added — manual deployment button visible in Actions tab</span>
            </label>
          )}
        </TaskCard>
        ...
        <TaskCard number="02" title="Add a deployment job to the pipeline" done={task2Done} locked={!task1Done}>
          <MentorNote>
            <p className="text-sm text-gray-600 leading-relaxed"><span className="text-gray-900">The deployment job runs after tests pass.</span>{" "}It only runs on main — not on PRs. This ensures the pipeline is the gatekeeper, not a person.</p>
          </MentorNote>
          <div className="flex flex-col gap-2">
    ...
    - name: Deploy
      run: |
        ...
        echo "Deploy complete."`}</CodeBlock>
          ...
          {!task2Done && (
            <label className="flex items-center gap-3 cursor-pointer group">
              ...
              <span className="text-sm transition-colors" style={{ color: "var(--text-muted)" }}>Deployment job added — runs automatically after green tests on main</span>
            </label>
          )}
        </TaskCard>
        ...
        <TaskCard number="03" title="Add deployment notifications" done={task3Done} locked={!task2Done}>
          <MentorNote>
            <p className="text-sm text-gray-600 leading-relaxed"><span className="text-gray-900">Visibility is part of safety.</span>{" "}When anyone can deploy, everyone needs to know when a deployment happens. A Git tag records who deployed, when, and from which commit.</p>
          </MentorNote>
          <div className="flex flex-col gap-2">
      ...
      run: |
        ...
        git push origin --tags`}</CodeBlock>
          ...
          {!task3Done && (
            <label className="flex items-center gap-3 cursor-pointer group">
              ...
              <span className="text-sm transition-colors" style={{ color: "var(--text-muted)" }}>Deployment creates a Git tag — every deployment is recorded</span>
            </label>
          )}
        </TaskCard>
        ...
        <TaskCard number="04" title="Create a DEPLOYMENT.md runbook" done={task4Done} locked={!task3Done}>
          <MentorNote>
            <p className="text-sm text-gray-600 leading-relaxed"><span className="text-gray-900">Self-service requires documentation.</span>{" "}An engineer should be able to deploy without asking anyone — which means the process must be written down and findable in the repo.</p>
          </MentorNote>
          <div className="flex flex-col gap-2">
...
The pipeline enforces all safety checks.`}</CodeBlock>
          ...
          {!task4Done && (
            <label className="flex items-center gap-3 cursor-pointer group">
              ...
              <span className="text-sm transition-colors" style={{ color: "var(--text-muted)" }}>DEPLOYMENT.md created — anyone can deploy by following the guide</span>
            </label>
          )}
        </TaskCard>
        ...
        <TaskCard number="05" title="Commit and push" done={task5Done} locked={!task4Done}>
          <MentorNote>
            <p className="text-sm text-gray-600 leading-relaxed"><span className="text-gray-900">Deploying no longer requires Marco.</span>{" "}Any engineer with repo access can trigger a deployment from the Actions tab. The audit trail shows who did it.</p>
          </MentorNote>
          <div className="flex flex-col gap-2">
          ...
          {!task5Done && (
            <div className="flex flex-col gap-3">
              ...
              <label className="flex items-center gap-3 cursor-pointer group">
                ...
                <span className="text-sm transition-colors" style={{ color: "var(--text-muted)" }}>Pipeline is green — self-service deployment is live</span>
              </label>
            </div>
          )}
        </TaskCard>
