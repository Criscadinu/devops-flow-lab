    ...
    <div className="flex-1 px-6 py-14">
      <div className="max-w-3xl mx-auto flex flex-col gap-8">
        ...
        <div className="flex flex-col gap-2">
          <h2 className="text-3xl text-gray-900 tracking-tight" style={{ fontFamily: "var(--font-heading)", fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.02em" }}>Your Mission - Complete the Loop</h2>
          <p className="text-sm leading-relaxed" style={{ color: "var(--text-muted)" }}>Auto-deploy to staging, approval-gate production, verify health, add deployment traceability.</p>
        </div>
        ...
        <div className="flex flex-col gap-5 p-6 border" style={{ backgroundColor: "var(--bg-card)", borderColor: "rgba(251,146,60,0.3)", borderLeft: "3px solid rgb(251,146,60)" }}>
          ...
          <p className="text-sm leading-relaxed" style={{ color: "var(--text-muted)" }}>This mission builds on M-17. Your pipeline should already have a manual deployment trigger and deployment tags in place.</p>
        </div>

        <TaskCard number="01" title="Add automatic deployment to staging on every green commit" done={task1Done} locked={false}>
          <MentorNote>
            <p className="text-sm text-gray-600 leading-relaxed"><span className="text-gray-900">Staging is the safe target for automatic deployment.</span>{" "}It matches production, is not customer-facing, and gives you confidence before promoting.</p>
          </MentorNote>
          <div className="flex flex-col gap-2">
    ...
    - name: Verify staging health
      run: |
        ...
        curl --fail http://localhost:3001/health || exit 1`}</CodeBlock>
          ...
          {!task1Done && (
            <label className="flex items-center gap-3 cursor-pointer group">
              ...
              <span className="text-sm transition-colors" style={{ color: "var(--text-muted)" }}>Staging deploys automatically on every green commit to main</span>
            </label>
          )}
        </TaskCard>
        ...
        <TaskCard number="02" title="Add a production deployment with manual approval" done={task2Done} locked={!task1Done}>
          <MentorNote>
            <p className="text-sm text-gray-600 leading-relaxed"><span className="text-gray-900">Production deployment should require a human decision — but not a human execution.</span>{" "}The pipeline does the work. A team member approves it.</p>
          </MentorNote>
          <div className="flex flex-col gap-2">
            <SectionLabel>Add to .github/workflows/ci.yml</SectionLabel>
    ...
    - name: Verify production health
      run: |
        ...
        curl --fail http://localhost:3002/health || exit 1`}</CodeBlock>
          ...
          {!task2Done && (
            <label className="flex items-center gap-3 cursor-pointer group">
              ...
              <span className="text-sm transition-colors" style={{ color: "var(--text-muted)" }}>Production deployment requires approval — pipeline does the work, human approves</span>
            </label>
          )}
        </TaskCard>
        ...
        <TaskCard number="03" title="Add deployment verification" done={task3Done} locked={!task2Done}>
          <MentorNote>
            <p className="text-sm text-gray-600 leading-relaxed"><span className="text-gray-900">A deployment that does not verify it worked is a hope, not a deployment.</span>{" "}The health check confirms the service is actually running after every deployment.</p>
          </MentorNote>
          <div className="flex flex-col gap-2">
...
curl http://localhost:3002/health`}</CodeBlock>
          ...
          {!task3Done && (
            <label className="flex items-center gap-3 cursor-pointer group">
              ...
              <span className="text-sm transition-colors" style={{ color: "var(--text-muted)" }}>Health check verifies deployment succeeded — failed health check fails the pipeline</span>
            </label>
          )}
        </TaskCard>
        ...
        <TaskCard number="04" title="Add deployment history to the app" done={task4Done} locked={!task3Done}>
          <MentorNote>
            <p className="text-sm text-gray-600 leading-relaxed"><span className="text-gray-900">Every deployment should be traceable — who deployed, when, and what commit.</span>{" "}This audit trail makes self-service safe.</p>
          </MentorNote>
          <div className="flex flex-col gap-2">
...
- DEPLOYED_BY=\${GITHUB_ACTOR:-local}`}</CodeBlock>
          ...
          {!task4Done && (
            <label className="flex items-center gap-3 cursor-pointer group">
              ...
              <span className="text-sm transition-colors" style={{ color: "var(--text-muted)" }}>GET /api/deployment returns deployment metadata</span>
            </label>
          )}
        </TaskCard>
        ...
        <TaskCard number="05" title="Commit and push" done={task5Done} locked={!task4Done}>
          <MentorNote>
            <p className="text-sm text-gray-600 leading-relaxed"><span className="text-gray-900">The loop is complete.</span>{" "}Every commit that passes tests deploys to staging automatically. Production requires approval. Every deployment is verified and traceable.</p>
          </MentorNote>
          <div className="flex flex-col gap-2">
          ...
          {!task5Done && (
            <div className="flex flex-col gap-3">
              ...
              <label className="flex items-center gap-3 cursor-pointer group">
                ...
                <span className="text-sm transition-colors" style={{ color: "var(--text-muted)" }}>Pipeline is green — deploy stages are integrated</span>
              </label>
            </div>
          )}
        </TaskCard>
