    ...
    <div className="flex-1 px-6 py-14">
      <div className="max-w-3xl mx-auto flex flex-col gap-8">
        ...
        <div className="flex flex-col gap-2">
          <h2 className="text-3xl text-white tracking-tight" style={{ ...syne.style, fontWeight: 800 }}>Your Mission - Automate the Checklist</h2>
        ...
        <TaskCard number="01" title="Document your manual test checklist" done={task1Done} locked={false}>
          <MentorNote>
            <p className="text-sm text-gray-300 leading-relaxed"><span className="text-white">Before you automate, you need to know what you are automating.</span>{" "}Write down every manual step you currently perform before a release. The checklist is the specification.</p>
          </MentorNote>
          <div className="flex flex-col gap-2">
...
6. GET /api/postmortems — returns postmortem list`}</CodeBlock>
          ...
          {!task1Done && (
            <label className="flex items-center gap-3 cursor-pointer group">
              ...
              <span className="text-sm text-gray-400 group-hover:text-gray-300 transition-colors">Manual test checklist documented</span>
            </label>
          )}
        </TaskCard>
        ...
        <TaskCard number="02" title="Convert checklist to integration tests" done={task2Done} locked={!task1Done}>
          <MentorNote>
            <p className="text-sm text-gray-300 leading-relaxed"><span className="text-white">Each item on the checklist becomes a test.</span>{" "}Use supertest to make real HTTP calls against the running app — no mocks, no assumptions.</p>
          </MentorNote>
          <div className="flex flex-col gap-2">
    ...
    expect(res.status).toBe(400)
          ...
          {!task2Done && (
            <label className="flex items-center gap-3 cursor-pointer group">
              ...
              <span className="text-sm text-gray-400 group-hover:text-gray-300 transition-colors">Release checklist converted to automated integration tests</span>
            </label>
          )}
        </TaskCard>
        ...
        <TaskCard number="03" title="Add supertest and export the app" done={task3Done} locked={!task2Done}>
          <MentorNote>
            <p className="text-sm text-gray-300 leading-relaxed"><span className="text-white">Supertest lets you make HTTP calls against your Express app without starting a real server.</span>{" "}The app must be exported from index.js to be testable.</p>
          </MentorNote>
          <div className="flex flex-col gap-2">
...
module.exports = app`}</CodeBlock>
          ...
          {!task3Done && (
            <label className="flex items-center gap-3 cursor-pointer group">
              ...
              <span className="text-sm text-gray-400 group-hover:text-gray-300 transition-colors">supertest installed, app exported for testing</span>
            </label>
          )}
        </TaskCard>
        ...
        <TaskCard number="04" title="Delete the manual checklist from your release process" done={task4Done} locked={!task3Done}>
          <MentorNote>
            <p className="text-sm text-gray-300 leading-relaxed"><span className="text-white">The automation replaces the checklist — it does not supplement it.</span>{" "}If both exist, engineers will skip the automation when they are in a hurry. Remove the manual step.</p>
          </MentorNote>
          <div className="flex flex-col gap-2">
...
Automated. Run \`npm run test:integration\` or let CI handle it.`}</CodeBlock>
          ...
          {!task4Done && (
            <label className="flex items-center gap-3 cursor-pointer group">
              ...
              <span className="text-sm text-gray-400 group-hover:text-gray-300 transition-colors">Manual checklist removed from release process — automation is the process</span>
            </label>
          )}
        </TaskCard>
        ...
        <TaskCard number="05" title="Commit and push" done={task5Done} locked={!task4Done}>
          <MentorNote>
            <p className="text-sm text-gray-300 leading-relaxed"><span className="text-white">Kai&apos;s two-day release ritual is now a 30-second CI job.</span>{" "}It runs on every commit. It never gets tired. It never misses a step.</p>
          </MentorNote>
          <div className="flex flex-col gap-2">
          ...
          {!task5Done && (
            <div className="flex flex-col gap-3">
              ...
              <label className="flex items-center gap-3 cursor-pointer group">
                ...
                <span className="text-sm text-gray-400 group-hover:text-gray-300 transition-colors">Pipeline is green — release checklist is automated</span>
              </label>
            </div>
          )}
        </TaskCard>
