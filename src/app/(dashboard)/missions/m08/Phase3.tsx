    ...
    <div className="flex-1 px-6 py-14">
      <div className="max-w-3xl mx-auto flex flex-col gap-8">
        ...
        <div className="flex flex-col gap-2">
          <h2 className="text-3xl text-white tracking-tight" style={{ ...syne.style, fontWeight: 800 }}>Your Mission - Speed Up the Tests</h2>
        ...
        <TaskCard number="01" title="Measure your current test suite speed" done={task1Done} locked={false}>
          <MentorNote>
            <p className="text-sm text-gray-300 leading-relaxed"><span className="text-white">You cannot optimize what you do not measure.</span>{" "}Time your test suite and record it as a baseline before making any changes.</p>
          </MentorNote>
          <div className="flex flex-col gap-2">
...
- Runtime under 3 minutes in CI`}</CodeBlock>
          ...
          {!task1Done && (
            <label className="flex items-center gap-3 cursor-pointer group">
              ...
              <span className="text-sm text-gray-400 group-hover:text-gray-300 transition-colors">Test performance baseline recorded</span>
            </label>
          )}
        </TaskCard>
        ...
        <TaskCard number="02" title="Identify and fix slow tests" done={task2Done} locked={!task1Done}>
          <MentorNote>
            <p className="text-sm text-gray-300 leading-relaxed"><span className="text-white">One slow test can dominate the entire suite.</span>{" "}Find it and fix it before parallelizing — parallelization amplifies problems, it does not fix them.</p>
          </MentorNote>
          <div className="flex flex-col gap-2">
          ...
          {!task2Done && (
            <label className="flex items-center gap-3 cursor-pointer group">
              ...
              <span className="text-sm text-gray-400 group-hover:text-gray-300 transition-colors">Slowest tests identified — at least one optimized</span>
            </label>
          )}
        </TaskCard>
        ...
        <TaskCard number="03" title="Split tests into fast and slow categories" done={task3Done} locked={!task2Done}>
          <MentorNote>
            <p className="text-sm text-gray-300 leading-relaxed"><span className="text-white">Categorizing tests lets you run the fast gate first and fail quickly on obvious errors.</span>{" "}Unit tests should take seconds. Integration tests can take minutes.</p>
          </MentorNote>
          <div className="flex flex-col gap-2">
  ...
  "test:integration": "jest --testPathPattern=integration"
          ...
          {!task3Done && (
            <label className="flex items-center gap-3 cursor-pointer group">
              ...
              <span className="text-sm text-gray-400 group-hover:text-gray-300 transition-colors">Tests split into unit and integration categories</span>
            </label>
          )}
        </TaskCard>
        ...
        <TaskCard number="04" title="Add parallel test execution to CI" done={task4Done} locked={!task3Done}>
          <MentorNote>
            <p className="text-sm text-gray-300 leading-relaxed"><span className="text-white">GitHub Actions supports running jobs in parallel.</span>{" "}Split your test categories into separate jobs that run simultaneously instead of sequentially.</p>
          </MentorNote>
          <div className="flex flex-col gap-2">
      ...
      - run: npm run test:integration`}</CodeBlock>
          ...
          {!task4Done && (
            <label className="flex items-center gap-3 cursor-pointer group">
              ...
              <span className="text-sm text-gray-400 group-hover:text-gray-300 transition-colors">CI runs unit and integration tests in parallel</span>
            </label>
          )}
        </TaskCard>
        ...
        <TaskCard number="05" title="Commit and push" done={task5Done} locked={!task4Done}>
          <MentorNote>
            <p className="text-sm text-gray-300 leading-relaxed"><span className="text-white">The performance baseline, the test categories, and the parallel CI are now permanent.</span>{" "}Every future engineer on this repo inherits fast feedback by default.</p>
          </MentorNote>
          <div className="flex flex-col gap-2">
          ...
          {!task5Done && (
            <div className="flex flex-col gap-3">
              ...
              <label className="flex items-center gap-3 cursor-pointer group">
                ...
                <span className="text-sm text-gray-400 group-hover:text-gray-300 transition-colors">Pipeline is green — tests run in parallel</span>
              </label>
            </div>
          )}
        </TaskCard>
